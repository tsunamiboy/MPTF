...
$(document).ready ->
  ...
  ws.onmessage = (event) ->
    ...
    feedTweet data
    mapBomb data
    d = -> Date.parse("March 21, 2012")
    d()
    console.log "Time is #{d}"
#    d = Date.parse(currTime)
#    d = Date.parse("March 21, 2012")
#    velocity (bombCnt, tB, tE)

# velocity stuff

# velocity = (bombCnt, ...) ->
  # timeBegin = -> new Date(milliseconds)
  # timeEnd = -> new Date(milliseconds)
  # time()
  # console.log "Time stands at #{ti}"
...
feedTweet = (tweet) ->
  console.log "Bomb count stands at #{bombCnt}"
